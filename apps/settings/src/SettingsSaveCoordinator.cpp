...
void SettingsSaveCoordinator::save() {
  ...
  if (appearance_->isDirty()) {
    ...
    if (result == AppearanceFileService::SaveResult::Success)
      ...
      ++failed;
      if (result == AppearanceFileService::SaveResult::Conflict) setConflict(QStringLiteral("Appearance"));
...
void SettingsSaveCoordinator::discard() {
  ...
  if (shell_->isDirty() && !shell_files_->load()) errors << QStringLiteral("Shell settings: ") + shell_files_->error();
...
void SettingsSaveCoordinator::reloadConflict() {
  if (busy_ || conflict_domain_.isEmpty()) return;
  setBusy(true);
  bool ok = conflict_domain_ == QStringLiteral("Appearance") ? appearance_files_->load() : shell_files_->load();
  setResult(ok ? QStringLiteral("External changes loaded") : QStringLiteral("Reload failed"));
  if (ok) setConflict({});
  setBusy(false);
}
void SettingsSaveCoordinator::overwriteConflict() {
  ...
  if (conflict_domain_ == QStringLiteral("Appearance"))
    ok = appearance_files_->save(true) == AppearanceFileService::SaveResult::Success;
  else
    ok = shell_files_->save(true) == ShellConfigFileService::SaveResult::Success;
  setResult(ok ? QStringLiteral("External changes overwritten")
