...
bool MissingDefaults::any() const {
  return ui_font || ui_font_size || fixed_font || fixed_font_size || clock_font || clock_font_size || title_font ||
         title_font_size || transparency || blur_strength || workspace_count || tray_max_items || background_images ||
         weather_api_key || weather_geo_api_key || weather_units || weather_lang || weather_refresh_interval ||
         notif_default_timeout || notif_max_visible || notif_history_enabled || notif_history_max_items ||
         notif_history_max_age_days || notif_history_persist_body;
}
...
AppearanceConfig parseAppearance(const toml::table& tbl, MissingDefaults& missing) {
  AppearanceConfig cfg;
  const auto sec = tbl["appearance"];
  cfg.ui_font = readStr(sec["ui_font"], cfg.ui_font, "appearance.ui_font", missing.ui_font);
  cfg.ui_font_size =
      readPositiveInt(sec["ui_font_size"], cfg.ui_font_size, "appearance.ui_font_size", missing.ui_font_size);
  cfg.fixed_font = readStr(sec["fixed_font"], cfg.fixed_font, "appearance.fixed_font", missing.fixed_font);
  cfg.fixed_font_size = readPositiveInt(sec["fixed_font_size"], cfg.fixed_font_size, "appearance.fixed_font_size",
                                        missing.fixed_font_size);
  cfg.clock_font = readStr(sec["clock_font"], cfg.clock_font, "appearance.clock_font", missing.clock_font);
  cfg.clock_font_size = readPositiveInt(sec["clock_font_size"], cfg.clock_font_size, "appearance.clock_font_size",
                                        missing.clock_font_size);
  cfg.title_font = readStr(sec["title_font"], cfg.title_font, "appearance.title_font", missing.title_font);
  cfg.title_font_size = readPositiveInt(sec["title_font_size"], cfg.title_font_size, "appearance.title_font_size",
                                        missing.title_font_size);
  cfg.transparency = readInt(sec["transparency"], cfg.transparency, "appearance.transparency", missing.transparency);
  cfg.transparency = clampRange(cfg.transparency, AppearanceConfig::kMinTransparency,
                                AppearanceConfig::kMaxTransparency, "appearance.transparency");
  cfg.blur_strength =
      readInt(sec["blur_strength"], cfg.blur_strength, "appearance.blur_strength", missing.blur_strength);
  cfg.blur_strength = clampRange(cfg.blur_strength, AppearanceConfig::kMinBlurStrength,
                                 AppearanceConfig::kMaxBlurStrength, "appearance.blur_strength");
  return cfg;
}
...
QStringList appearanceDefaultLines(const MissingDefaults& missing) {
  const AppearanceConfig appearance;
  QStringList lines;
  if (missing.ui_font) {
    lines << QStringLiteral("ui_font = %1").arg(tomlQuote(appearance.ui_font));
  }
  if (missing.ui_font_size) {
    lines << QStringLiteral("ui_font_size = %1").arg(appearance.ui_font_size);
  }
  if (missing.fixed_font) {
    lines << QStringLiteral("fixed_font = %1").arg(tomlQuote(appearance.fixed_font));
  }
  if (missing.fixed_font_size) {
    lines << QStringLiteral("fixed_font_size = %1").arg(appearance.fixed_font_size);
  }
  if (missing.clock_font) {
    lines << QStringLiteral("clock_font = %1").arg(tomlQuote(appearance.clock_font));
  }
  if (missing.clock_font_size) {
    lines << QStringLiteral("clock_font_size = %1").arg(appearance.clock_font_size);
  }
  if (missing.title_font) {
    lines << QStringLiteral("title_font = %1").arg(tomlQuote(appearance.title_font));
  }
  if (missing.title_font_size) {
    lines << QStringLiteral("title_font_size = %1").arg(appearance.title_font_size);
  }
  if (missing.transparency) {
    lines << QStringLiteral("transparency = %1 # percent; accepted: %2-%3")
                 .arg(appearance.transparency)
                 .arg(AppearanceConfig::kMinTransparency)
                 .arg(AppearanceConfig::kMaxTransparency);
  }
  if (missing.blur_strength) {
    lines << QStringLiteral("blur_strength = %1 # px; accepted: %2-%3")
                 .arg(appearance.blur_strength)
                 .arg(AppearanceConfig::kMinBlurStrength)
                 .arg(AppearanceConfig::kMaxBlurStrength);
  }
  return lines;
}
...
QStringList defaultLinesForSection(const QString& section, const MissingDefaults& missing) {
  ...
  if (section == QLatin1String("appearance")) {
    lines = appearanceDefaultLines(missing);
...
ParsedConfig parseConfigTable(const toml::table& table, MissingDefaults& missing) {
  ParsedConfig parsed;
  parsed.appearance = parseAppearance(table, missing);
...
bool writeMissingDefaults(const QString& path, const MissingDefaults& missing) {
  ...
  insertMissingSectionDefaults(lines, QStringLiteral("appearance"),
                               defaultLinesForSection(QStringLiteral("appearance"), missing));
