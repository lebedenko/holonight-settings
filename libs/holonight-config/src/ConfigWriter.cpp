...
bool ConfigWriter::write(const ParsedConfig& config, const QString& path) {
  ...
  if (!dir.mkpath(QStringLiteral("."))) {
    qCWarning(lcConfigWriter) << "Failed to create config directory:" << dir.absolutePath();
    return false;
  }

  QSaveFile file(path);
  if (!file.open(QIODevice::WriteOnly | QIODevice::Truncate | QIODevice::Text)) {
    qCWarning(lcConfigWriter) << "Failed to open config for writing:" << path;
    return false;
  }
  ...
  out << "[appearance]\n";
  out << "ui_font = " << tomlQuote(config.appearance.ui_font) << "\n";
  out << "ui_font_size = " << config.appearance.ui_font_size << "\n";
  out << "fixed_font = " << tomlQuote(config.appearance.fixed_font) << "\n";
  out << "fixed_font_size = " << config.appearance.fixed_font_size << "\n";
  out << "clock_font = " << tomlQuote(config.appearance.clock_font) << "\n";
  out << "clock_font_size = " << config.appearance.clock_font_size << "\n";
  out << "title_font = " << tomlQuote(config.appearance.title_font) << "\n";
  out << "title_font_size = " << config.appearance.title_font_size << "\n";
  out << "transparency = " << config.appearance.transparency << " # accepted: " << AppearanceConfig::kMinTransparency
      << "-" << AppearanceConfig::kMaxTransparency << "\n";
  out << "blur_strength = " << config.appearance.blur_strength << " # accepted: " << AppearanceConfig::kMinBlurStrength
      << "-" << AppearanceConfig::kMaxBlurStrength << "\n\n";
  ...
  if (!file.commit()) {
    qCWarning(lcConfigWriter) << "Failed to commit config:" << path;
    return false;
  }
  return true;
}
