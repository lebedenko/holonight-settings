...
"""Enforce the UQC provider's application import boundary in settings QML."""
import re
import sys
...
def main():
    root = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).resolve().parents[1] / 'apps/settings/qml'
